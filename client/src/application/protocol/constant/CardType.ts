...
class CardType {
    ...
    public static isDouble(cards: CardDto[]): boolean {
        if(cards.length == 2 && cards[0].weight == cards[1].weight) return true;
        return false;
    }
    /**
     * 是否为顺子
    ...
    public static isDoubleStraight(cards: CardDto[]) {
        ...
        for(let i = 0; i < cards.length - 2; i+=2) {
            ...
            if(cards[i+2].weight - cards[i].weight != 1) return false;
            if(cards[i].weight > CardWeight.ONE || cards[i+1].weight > CardWeight.ONE) return false;
        }
        return true;
    }
    /**
    ...
    public static isTripleStraight(cards: CardDto[]) {
        ...
        for(let i = 0; i < cards.length - 3; i+= 3) {
            if(cards[i].weight != cards[i+1].weight || cards[i+1].weight != cards[i+1].weight) return false;
            if(cards[i+3].weight - cards[i].weight != 1) return false;
            if(cards[i].weight > CardWeight.ONE || cards[i+3].weight > CardWeight.ONE) return false;
        }
        return true;
    }
    /**
    ...
    public static isThree(cards: CardDto[]) {
        ...
        if(cards[0].weight != cards[1].weight || cards[1].weight != cards[2].weight) return false;
        return true;
    }
    /**
     * 是否为三带一 333 4(默认已排好序)
    ...
    public static isThreeAndOne(cards: CardDto[]) {
        ...
        if(cards[0].weight == cards[1].weight && cards[1].weight == cards[2].weight && cards[2].weight != cards[3].weight) return true;
        if(cards[1].weight == cards[2].weight && cards[2].weight == cards[3].weight  && cards[0].weight != cards[1].weight) return true;
        return false;
    }
    /**
     * 是否为三带二
    ...
    public static isJokerBoom(cards: CardDto[]) {
        ...
        if(cards[0].weight == CardWeight.SJOKER && cards[1].weight == CardWeight.LJOKER) return true;
        if(cards[0].weight == CardWeight.LJOKER && cards[1].weight == CardWeight.SJOKER) return true;
        return false;
    }
